...
def pose_encoding_to_extri_intri(
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        fov_h = torch.clamp(fov_h, min=0.09, max=2.97) # Clamp FOV to safe range: ~5° to ~170°
        fov_w = torch.clamp(fov_w, min=0.09, max=2.97) 
        ...
        if build_intrinsics:
            ...
            if equal_view_intrinsics:
                fy = fy[:,0:1].repeat(1,pose_encoding.shape[1])
                fx = fx[:,0:1].repeat(1,pose_encoding.shape[1])
